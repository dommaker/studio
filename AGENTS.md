...
- 约束清单：`CLAUDE.md` Governance Rules 块（Iron Laws 12 条、Guidelines 28 条）

## 知识入口

- `.harness/knowledge/`：项目知识库（757 条），用 `harness knowledge` 查询
- 各源码目录的 `CONTEXT.md` 是权威模块文档（现有 49 个），改动代码时同步更新

<!-- PRESERVE:overview -->
## 项目概述

Studio 是多 Agent 群聊控制台（薄编排层），只做三件事：任务派发（人在频道 @角色 提需求，系统派给 agent CLI 执行）、状态监控（任务/角色/工程状态与 token 开销可观测）、知识飞轮（执行数据沉淀为知识，再注入后续任务）。架构宪法见 `docs/vision-2026.md`；关键机制决策见 `docs/plans/2026-07-27-agents-md-skill-governance.md`。

### 主干链路

频道消息 → message-routing（@mention 匹配频道成员；无 @ 时派给频道默认角色，未配置则只存消息）→ 创建 WorkUnit → AgentLoop observe→claim→step → step 时拼注入段并 spawn agent CLI（claude/kimi/codex/opencode）在目标工程的 worktree 执行 → COMPLETE / NEED_INPUT 回写频道 → 执行数据落盘，飞轮提取（LLM 提案 → 频道人审 → 生效）。

### 关键机制（2026-07-27 治理后口径）

- **认领纯显式**：指派给我的 + 我是频道成员的未指派任务，先 claim 先得；没有类型过滤。
- **skill 注入 = 索引 + 按需协议**：step 时按相关度排序（`+显式点名` > 角色 acceptedTypes 域匹配 > scope 文本匹配 > 热度），prompt 只带索引行（描述 + triggers + `~/.studio/skills/<name>/SKILL.md` 绝对路径），agent 命中触发条件才读全文。不向工作区写任何文件。
- **注入预算**：skills > persona（## 你的角色）> roster（## 频道成员与委派）> knowledge，共用 ≤2K tokens。
- **职能域**：角色 acceptedTypes 是 profile 显式字段（创建时可从 `.agents/roles/*.yaml` 预设带入），阶段词表 {design, plan, implement, test, review, docs, refactor, analysis, general}，legacy 值经 studio-shared 的 normalizeToStage 归一化。
- **skill 权威库** = `~/.studio/skills`（飞轮写入处，仓库内不存副本）；运行时知识库 = `~/.studio/knowledge`。

### 怎么下手（按任务类型）

- 派发 / 无@兜底 / 频道成员：apps/api/src/modules/channels（message-routing）
- 认领 / 注入 / prompt 组装：apps/api/src/modules/agents（agent-loop）
- skill 匹配与清单：apps/api/src/modules/skills（skill-selector、manifest-loader）
- 执行 / worktree / session：packages/studio-agent（services/runner-lightweight、worktree-resolver）
- 存储 / 事件 / 词表：packages/studio-shared（file-store、domain-vocab）
- 知识飞轮：apps/api/src/modules/knowledge；REQ 编号：requirements；文档新鲜度：sdd
- 模块级细节一律以各目录 CONTEXT.md 为准（本文件末「模块索引」有全量列表）

### 文档与治理约定

- 本文件 = harness 骨架（`harness sync-docs --agents` 生成）+ PRESERVE 手写段 + AUTO 模块索引段；重建用 `pnpm agents-md:sync`，CI 有漂移检查——PRESERVE 段外勿手改。
- 决策文档在 `docs/plans/`（该目录被 gitignore，属本地权威记录）；SDD 文档在 `docs/sdd/`。
- 治理配置 `.harness/config.yml`（preset standard）；约束清单见 CLAUDE.md Governance Rules 块。
<!-- /PRESERVE:overview -->
