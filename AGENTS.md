...
> 本文件由 `harness sync-docs --agents` 生成骨架；文末「模块索引」区段由 `pnpm gen:agents-md` 自动维护（见 AUTO-GENERATED 标记），请勿手改标记区段。
...
- `.harness/knowledge/`：项目知识库（631 条），用 `harness knowledge` 查询
...
| `packages/studio-shared` | 本目录是 Agent-Studio 的前后端共享库，提供 CLI 框架、配置管理、常量定义、事件总线与文件存储等通用基础设施，为 apps/api 等多个上层模块提供复用的工具与类型。 |
