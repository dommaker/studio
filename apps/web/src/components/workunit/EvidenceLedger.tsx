...
// variant 只承载真实差异：外层标记（mc-kv vs card）与 verdict 呈现（图标前缀 vs 通过/拒绝徽章）
              ...
              <span className="mc-kv-v">
                {entry ? (
                  <span className="inline-flex items-center gap-1">
                    {entry.verdict === 'approved' ? <IconCheck size={12} /> : <IconX size={12} />}
          ...
          {LEVELS.map(level => {
            ...
            return (
              <div className="flex items-center gap-2 text-xs" key={level}>
                ...
                {entry ? (
                  <>
                    <span className={`px-2 py-0.5 rounded inline-flex items-center gap-1 ${entry.verdict === 'approved' ? 'u-ok-dim u-ok' : 'u-err-dim u-err'}`}>
                      {entry.verdict === 'approved' ? <><IconCheck size={12} /> 通过</> : <><IconX size={12} /> 拒绝</>}
                    </span>
