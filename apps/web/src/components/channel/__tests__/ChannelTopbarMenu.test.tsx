...
// #474：「当前 PMO」chip 提升为顶栏可见位（ChannelDetailPage 直挂），菜单不再收纳 PMO 跳转
// 覆盖：默认收起 / 点击展开收纳位（频道动态/成员/工单路由/默认工程）/ 频道动态入口回调并收菜单 /
// 点外部收起 / 默认工程 Select 的 portal 面板（.select-panel）点击不收菜单 / 成员管理触发器换菜单行形态
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { render, screen, fireEvent } from '@testing-library/react';

// 收纳组件保留接口、隔离内部 API 依赖（成员/默认工程各自有独立测试）
...
describe('ChannelTopbarMenu — E1 顶栏 ⋯ 菜单', () => {
  ...
  it('默认收起；点 ⋯ 展开收纳位（频道动态/成员/默认工程；#474 PMO 已提升顶栏）', () => {
    renderMenu();
