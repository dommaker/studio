...
- **频道工作区**（#394 三栏定稿，spec §4.1–4.3）= 左 ChannelRail（200px，频道名 500 字重/未读加粗）/ 中对话流（Linear 式弹性：max-width 无上限、左对齐、padding 32px，760/1000 双档居中已删）/ 右 ChannelActivityRail（300px「频道动态」REQ 链路卡；<1024 过渡隐藏，顶栏入口归 #395）+ 覆盖式右抽屉 WorkUnitDrawer。原中栏 REQ chips 条已移除（REQ 呈现挪右栏链路卡）。消息分侧：人类右气泡、agent 左文档流、系统居中。
