...
- **频道流虚拟化**（#325，ADR 2026-08-24 channel-stream-virtualization；实施前为决策态）：方案 = 真 windowing（`@tanstack/react-virtual`，headless + 动态变高测量），一期仅渲染层、数据层裁剪归 #326。虚拟化下锚点语义：视口行必渲染不变式（`captureFirstVisibleAnchor` 不动）；行锚点补偿 = 唯一 scrollTop 校正权（virtualizer 自动调整须关/接管，实施前先验证）；阅读位置恢复两段式（`scrollToIndex` 粗定位 + 测量落地后 DOM 精校正）；钉底重定义为末行局部几何（不依赖总高度）；virtualizer 一切程序滚动必过 observed-top 台账。
