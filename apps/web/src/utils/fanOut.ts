...
// 收敛前端手写的 allSettled/逐条 try-catch 扇出样板；每条 fetcher 立即发出（无并发上限）。
// 调用方自带归并策略（丢弃/兜底 null/兜底行），取消（cancelled/alive）仍留在调用侧。
// ⚠ repo tsconfig strict:false：ok 是 boolean 字面量判别式，truthy 判断只收窄 true 分支，
// false 分支拿 error 必须显式 `entry.ok === true ? ... : entry.error`（truthy 会留在全联合上报 TS2339）。
