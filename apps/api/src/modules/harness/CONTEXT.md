...
| `constraints.routes.ts` | 约束清单 + 质量门（/constraints*、/check-constraints；degrade/schedule 已随 0.17.0 移除；条目字段随 harness 1.10.0 换 severity 显式面，stats 聚合桶 byLevel→bySeverity；retired/rollback 双落点——config.yml 内置/历史 + custom-constraints.yml #82 D6 落点（该文件已随 studio#606 从本仓退役，通道保留为可选历史面）；导出 customConstraintsPath 供 distill #146 审计装配复用） |
