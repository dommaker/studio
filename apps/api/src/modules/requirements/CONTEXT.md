...
- `ownership-resolver.ts` — B3a 工程归属解析（决策 D2 + #285 决策 #249 §4）：Requirement.projectId → PMO gitRepo > 文件引用（#285：fileRefs 全部引用尾斜杠归一后同仓 → source=file-refs、workspaceRoot=归一 repo；跨多仓不参与落下一 rung，空数组等同无引用）> 频道 defaultPath（默认工程）> none。#481（2026-09-11）：「显式 workspaceId」与「频道 defaultWorkspaceId（默认执行机器）」两级已退役——机器指针不再产出（OwnershipResolution 无 workspaceId 字段；OwnershipSource 无 explicit/channel-default），频道只配机器指针等同无归属。#525 P2-2：输入新增可选 `channel`——路由层已读快照传入时「频道 defaultPath」级不再重复 `getChannel`（未传入保持现状读）
...
- `wu-pmo-attribution.ts` — 2026-08 归因统一：创建期 PMO 归因戳纯解析叶子（零 app 依赖，防 pmo/ → pmo-branch-resolver → project.service → workunit.service 循环）；`parseWuPmoId` = metadata.pmoId（canonical）→ metadata.ownershipProjectId（deprecated legacy 同位）容错同步解析，pmo-branch-resolver、pmo/evidence-summary（内存过滤）、workunit/workunit-crud（#402 父戳继承）与 requirement.service（#402 解绑对账 warn）共用
- `rollup.ts` — REQ 状态汇总：订阅 `workunit.status_changed` 事件回写需求整体状态（别名视图跳过，PMO 侧 progress-rollup 拥有）。#457 起消费侧 per-req memo + 50ms 去抖（照 #410 progress-rollup 先例）：事件负载喂 memo 稳态零存储读，created 只记账不触发，冷启动首个评估经 `listWorkUnitSnapshots` 回源一次；`maybeRollUpToDone(reqId, knownSnapshots?)` 第二参直供快照时跳过全量索引读，Requirement 自身状态判定恒新鲜 get
...
- **GET / 无过滤即全量列表**（legacy REQ + PMO 别名视图合并，分页参数无——全量返回）：批次 D-2 项5 PMOPage「需求」索引 tab 消费（配合 chain-stats 批量计数），新增全量消费方注意单页规模。
