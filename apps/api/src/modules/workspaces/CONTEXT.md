...
- **'VPS' 命名约定的唯一属主在 studio-shared（2026-08 seam 修复）**：判定"哪条记录是本机 VPS workspace"（name='VPS' 且无 tokenId）由 `@dommaker/studio-shared/node` 的 `resolveVpsWorkspace()` 统一定义；本模块的 `local-workspace.findLocalWorkspace` 与 `GET /workspaces/runtimes` 都走它，不再各自手扫 JSON。改写存储格式/重命名 VPS workspace 时需同步该函数。2026-09-10 起 studio-agent worktree-resolver 不再是消费方——记录的 `workspaceRoot` 不再充当执行面 cwd 的隐式兜底（见 `packages/studio-agent/src/CONTEXT.md`「执行目录解析」）。
...
- **workspace token 子系统 = 已查清的纯残骸（2026-09-10 盘点，删它零行为变化）**：`workspaceAuth()`（`middleware/auth.ts`）唯一挂载点是已删的 register/heartbeat，现全仓库零路由引用；`token.routes.ts` 生成的 token 更早已不可用——磁盘 28 个 token 文件**全部缺 `workspaceId` 字段**，而 `workspaceAuth()` 靠该字段定位 workspace，即挂上路由也恒 401。UI 侧同样无入口：`workspaceTokenApi`（`apps/web/src/api/index.ts`）零 importer，WorkspacePage 只有 runtimes 列表 + 创建角色弹框。无读取方的数据目录：`~/.studio/workspace-tokens/`、`~/.studio/data/workspace-tokens/`、`~/.studio/data/workspaces/`（后两个属旧 STUDIO_DATA_DIR 根）。
- **承重面（勿删）**：`workspace.routes.ts` 的只读/删除端点 + `workspace-store.ts`（`resolveWorkspaceRoot`）——链路是 `channel.defaultWorkspaceId` → `wu.workspaceId` → agent-loop 解 cwd、wu-verification 取 workspace 级 `verifyCommands` 覆盖。记录缺失时 `resolveWorkspaceRoot` 返回 null、agent-loop 打 warn 回退默认 cwd（不报错）。生产分布：48 个 WU 快照 = 47 null + 1 个孤儿 `ws_e2e_p15`；3 个频道 = 2 null + 1 指向在用 VPS 记录。唯一还需产品拍板的是「默认执行机器」section 的去留：只剩一条 VPS 记录时它是「无 / VPS」二选一，且 VPS 的 root 与 `metadata.workspaceRoot` 优先路径同值，实际作用收敛为孤儿绑定提示 + verifyCommands 覆盖入口。
