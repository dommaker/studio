...
- 另知（未修，架构评审第三轮 N2）：`GET /workspaces/runtimes` 每次请求都 best-effort 全量重扫——`cli-scanner.scanAllProviders()` 对每个 provider 同步 `execFileSync` 跑 `which` + `--version`（timeout 5s/个），阻塞事件循环，最坏数十秒；前端三个挂载点（ChannelMemberManager/RolesSetup/CreateRoleModal）各自请求即各自扫一遍。决策已定（docs/adr/2026-08-31-channel-data-plane-store.md 决策 4/5）：前端懒挂载 + 本端点挂 apiCache 60s，待实施。
