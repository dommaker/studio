...
- **时间窗读口（#335，2026-08-25；#342 收敛全部度量读方，2026-08-27）**：`readStudioEventsSince({ sinceMs, file? })`（同在 studio-events-tail.ts，与 #180 共用字节层倒扫生成器）——尾部倒读、命中首个窗口外行即停扫，窗口外的行不 parse。**前提：文件 append-only 时间单调**（writeStudioEvent 恒追加、#173 轮转保序）；NaT 行跳过不停扫；返回文件序。已切换的读方：monitor-probes(1h)、monitor-reports×4(24h/7d)、auditor-reports(24h)、auditor-rules×2(28d)、evolution/signals(windowHours)、metrics.service×2(windowDays)、pattern-miner(24h)、wu-changed-files(30d 对齐热保留期)；#342 补齐：monitoring.service getOverheadStats(windowDays 默认 30d)、knowledge-metrics computeOutcomeMetrics/scanKnowledgeEvents(windowStart)、token-usage.service 三读点(30d，totals/树聚合/台账口径从「文件全量」收敛为 30d 窗口内、NaT 行不再计入累计)、session-summary-generator 两读点(30d，挂在 agent-events 批量写入 HTTP 路径上的读也随文件总量解耦)。**保持全量读**：monitor-lifecycle 的 precipitate/7d 截断/30d 清理三处——它们重写整个文件，必须拿到窗口外行。bench 数据集（bench/synthesize-dataset.ts）事件副本按 -k*12h 偏移保单调，否则早停前提在合成数据上不成立；fixture 里「窗口外噪音行」必须排文件头（最旧在前），同款微基准见 bench/read-since-windows-342.ts + report-read-sites-342.md。
