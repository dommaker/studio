 ...
 * AC C.3: goal.service.ts listGoals accepts and passes failureType to goal-crud
 */
...
describe('GoalService.listGoals — failureType pass-through (C.3)', () => {
  test('passes failureType to goal-crud listGoals', async () => {
    ...
    expect(mockFindMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          companyId: 'company-1',
          status: 'failed',
          GoalExecution: { some: { failureType: 'infrastructure' } },
        }),
      })
    );
  });
