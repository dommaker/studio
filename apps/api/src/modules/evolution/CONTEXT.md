...
- `applier.ts` — 提案生效器（审核通过后写入生效落点）。约束类（iron-law/guideline）
  **#602 D1 新落点 = `<repoRoot>/.harness/config.yml`**：仅支持 constraintChange='retire'
  （`constraints.<id>.enabled:false` + retired 墓碑，格式对齐 `harness constraints retire`；
  写后 getEffectiveConstraints 验证生效集已缩小，失败回滚备份；幂等 already retired）。
  message/new-entry/exception 在 harness 1.10.0 无生效落点，落笔前抛「落点已退役」。
...
- **约束类提案落点加闸（2026-09-21 上午加闸，同日 #602 D1 裁定新落点后收窄）**：落笔前抛错闸仍对 message/new-entry/exception 生效（无消费端）；retire 已走 config.yml 真实落点。
