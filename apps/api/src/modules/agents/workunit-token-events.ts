// workunit:tokens / tool:call 事件记录（M2 成本红线度量 + B6 真实 token 记账）——
...
/** 度量事件写入共享的 FileStore 实例（workunit:tokens 与 knowledge:skill_used 同一写入面） */
...
export function studioEventsJsonlPath(): string {
