...
import { isProcessAlive } from '../loop/agent-loop-parsers';
import { writeToolCallEvents, resolveToolTraceFile } from '../loop/agent-loop-events';
...
describe('AgentLoop', () => {
  ...
  // #363：原「Terminated history cleanup (2026-07-30)」describe 块随 start() 同角色
  // 启动清理一并拆除——回收职责统一归 instance-timeout-scan（见
  // __tests__/instance-timeout-scan.test.ts 的 #363 组）。

  describe('#363: start() 不再承担 terminated 回收', () => {
    it('同角色 terminated 历史实例经 start() 后原样保留（由 scan 统一回收）', async () => {
      ...
      const oldTerm = await fileStore.getState('old-term-1');
      expect(oldTerm).not.toBeNull();
      expect(oldTerm!.status).toBe('terminated');
    });
  });
