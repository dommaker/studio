...
- `ops/` - ops.service（preflight 启动检查/health 轮询自愈/代理守护/worktree GC/默认数据 ensure）、createHealthRoutes（/healthz 健康端点）、proc-probes（#344：/proc 系统探测单出口——statfs 磁盘/meminfo 内存//proc/*/stat 僵尸//proc/loadavg，零子进程；ops.getStatus 与 monitor systemHealthCheck 同源委托）
