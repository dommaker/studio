...
- **user-model 状态文件路径唯一出口（2026-09-08，ship-chain 阶段2 体检）**：harness `update-user-model` 写的状态文件，本模块读侧 `auditor-rules.analyzeUserModel` 与写侧 `auditor-execution.applyLowRiskSuggestions`（`model_weight_tune` 分支）**统一走 `auditor-rules.userModelStateFile()`**，不得再各自 `os.homedir()` 拼路径。该函数语义须与 harness 上游 `resolveUserModelPaths` 严格同形——**用 `||` 不用 `??`**：harness 把 `HARNESS_UUM_STATE_FILE` 设成空串时落回默认路径，若本仓用 `??` 就会读到 `""`，两边读不同文件 → `analyzeUserModel` 静默返回空建议、tune 分支静默不生效且无报错（`auditor-rules.test.ts` 的「env 为空串 → 落回默认」与端到端「状态文件在覆盖路径下仍能读到」两条用例即此锁）。
