...
- `review-contract.ts` — 审查结论（verdict）语义单一来源（2026-08 收编）：`ReviewVerdict`（pass/reject/needs-info）与 issue 词表 + legacy `{overallApproved}` ↔ 规范 verdict 映射（true⇔pass / false⇔reject / needs-info 无 legacy 等价 → null）+ 规范裁决规则 `deriveVerdictFromLegacyReport`（任一 error 级 issue ⇒ 不 pass，原 review.service FIX #8 收编于此）。消费方：review-report.ts（CodeQualityIssue severity）、review.service.ts（裁决入口）、agent-loop parseReviewReport（返回类型 ParsedReviewReport）、review-dispatcher.ts（路径 B 消费同型）
...
- **F6 台账 l1（决策 1）**：COMPLETE 前自动验证守卫同时写 `metadata.attestations.l1`（approved/rejected 都落，by=profile id；守卫实现见 `completion-gates.ts`）
...
- **鉴权（2026-07-24 收紧）**：legacy agents POST `/`、PUT `/:agentId` 与 agent-profiles/agent-instances 写 = `requireAuth()+requireNotGuest()`；`POST /review/diff`（任意路径写+spawn claude）与 instances `POST /:id/terminate` = `requireAuth()+requireAdmin()`；legacy DELETE 原有 requireRole('Admin') 不变。另知：agent-configs `:id` 路径拼接无校验（穿越面，未修）；/review/diff 的 baseRef/headRef/repoPath shell 拼接**已修**（2026-08，review.service `shellQuote` 单引号转义 + 回归测试，execSh 走 bash -c 无 arg 数组形态，转义是唯一安全插值）
