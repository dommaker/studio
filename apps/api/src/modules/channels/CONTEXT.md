...
- **频道建议派生端点（#443，spec #441 情境引导 02）**：`suggestions.ts deriveChannelSuggestions` + `GET /:id/suggestions`（匿名只读，仿 current-pmo）——不落库按当前事实现算，fail-closed（前置不满足/拿不准/读取失败 → 空结果不抛出）。输入 = 频道 WU 快照（getIndex：review 子单存在性/租约活性、updatedAt）+ loop 活跃度（复用 agents `summarizeRoleStates` 心跳聚合，5min 窗口单源 INSTANCE_ALIVE_TIMEOUT_MS 不另设）+ 频道 members；状态列口径 = shared deriveDisplayState；「当前工单」拣选后端化（**排除 review 子单**——自动化内部执行单元不作引导对象，否则父单 in_review 时子单 updatedAt 恒新、状态说明永不触发；只剩子单时回退全集）。宽限期集中 `SUGGESTION_TIMING`（reviewDispatchInFlightGraceMs=5min，对齐对账扫描档）。返回 `{ currentWuId, suggestions: [{ id, kind, params }] }`——只出结构化数据，文案由前端模板渲染。status 形态（#443，`auto-review-in-flight`：有未完结 review 子单且（租约活或有在线 loop），或无子单但宽限内有在线 loop）；action 形态第一种 = #444（见下条），prompt 后端化 = #446
