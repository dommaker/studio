...
- **消息分页端点（#319，2026-08-24）**：`GET /:id/messages` 的 `before` 游标 = 锚点**消息 id**（不再是 timestamp——同毫秒多条会漏/重），分页下沉到 `FileStore.queryMessagesPage`（存储层切片）；锚点 id 不存在时返回空页 + hasMore=false。前端 `useChannelEvents.loadMore` 传 `oldest.id`。消息存储压实机制与 id 新鲜度契约见 studio-shared CONTEXT.md
