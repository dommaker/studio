...
export async function buildAnalystPrompt(requirement: string, knowledge: string, accuracyReflection: string, outputFile: string, preClassifiedTier?: string, availableRepos?: RepoInfo[]): Promise<string> {
  ...
  return [
    ...
    '',
    '**铁律：只输出用户明确要求的需求。** 代码探索中发现的改进机会（如"这个模块可以升级"）→ 通过 gotchas/codePatterns 字段输出（自动进入 KnowledgeStore），不创建额外的 RequirementsDoc。',
    '',
    '**铁律：AC 描述必须动词开头。** "在 X 中添加 Y" ✅ | "X 中的 Y 功能" ❌。不符合的 AC 会被 RequirementGate 驳回。',
    '',
    '**铁律：已实现的需求不创建 WorkUnit。** 探索代码后发现需求描述的功能已完整实现 → 输出 `{ "title": "...", "summary": "已实现: [证据]", "acGroups": [], "contractTests": [], "contractTestsSkipReason": "已实现: [具体证据如文件路径+函数名]", "constraints": [], "tags": [] }`，不创建 RequirementsDoc。部分已实现 → 只列未实现的 AC。',
    '',
    '## 分析深度分级（先评估任务复杂度，再决定探索深度）',
    '**Simple**（改 1-2 文件，无新建文件，无 schema 变更）：',
    '  → 只读目标文件 + 直接调用方，30s 内完成分析',
    '**Medium**（改 3-5 文件，有新建文件，跨模块）：',
    '  → 读相关模块 + 调用链，2min 内完成',
    '**Complex**（新模块、auth/migration/schema 变更、架构重构）：',
    '  → 完整架构上下文 + 全库探索',
    '',
    '**architectureContext 所有 tier 统一标准**：无论 Simple/Medium/Complex，都必须填写完整的 architectureContext（functions 带行号、callChain、imports、typesInScope、testMock、dangerZones、verifiedAt）。Executor 是 fast 模型，依赖 architectureContext 定位改动位置，不探索代码库。',
    '',
    ...
    ...(availableRepos && availableRepos.length > 0 ? [
      ...
      '- **同一 WorkUnit 的所有 acGroup 必须属于同一仓库**。涉及多个仓库时，将不同仓库的 acGroup 分为独立的 "仓库组"',
      '',
    ...
    '5. 按文件独立性和语义依赖拆分为 AC 组（三层过滤：文件重叠→语义依赖→独立性）。**每组 ≤5 AC**（硬限，gate 会拒绝 >5）。复杂需求拆为多组',
    ...
    '   - functions: 关联函数签名+行号（如 handleWorkUnitCompleted(workUnitId): Promise<void> @ L612）',
    ...
    '- 将代码库发现通过 gotchas/codePatterns 字段输出（自动进入 KnowledgeStore 供未来分析复用）',
    ...
    '1. **文件路径**：精确到文件（如 `apps/api/src/modules/workunit/workunit.service.ts`）',
    '2. **位置**：行号范围或锚点（如 `L120-L145` 或 `在 handleWorkUnitCompleted() 方法后`）',
    ...
    '',
    '示例 AC：',
    '`在 workunit.service.ts L250-L264；添加 Resolution 查询和 LLM 兜底；Resolution 无匹配时尝试 LLM，LLM 不可用时静默降级；不修改 create() 的成功路径`',
    '',
    '  - **每个 acGroup 文件数 ≤ 5**：超过 5 个文件必须拆分为多个 AC 组（硬质量门，超限会被拒绝）',
    '  - **每个 AC 只含一个验证点**：不可组合多个独立功能到一个 AC',
    '  - **architectureContext 要求**：精确到文件+行号+函数签名（fast 模型不探索代码库，完全依赖此信息）',
    '',
    '## 输出格式',
    `将 RequirementsDoc JSON 写入 ${outputFile}（三段结构：requirement / design / task）：`,
    '```json',
    '{',
    '  "requirement": {',
    '    "title": "需求标题",',
    '    "summary": "一句话总结",',
    '    "tier": "fast",',
    '    "tierReason": "Executor 始终使用 fast 模型",',
    ...
    '  "design": {',
    '    "acGroups": [{',
    '      "id": "组名（与 requirement.acGroups 对应）",',
    '      "implementationNotes": "实现指南：步骤+函数名+关键决策",',
    '      "architectureContext": {',
    '        "functions": ["handleWorkUnitCompleted(workUnitId: string): Promise<void> @ L612"],',
    '        "callChain": "checkWorkUnitCompletion() → handleWorkUnitCompleted() → finalizeWorkUnit()",',
    '        "imports": ["import { logger } from \'@dommaker/studio-shared\'"],',
    '        "typesInScope": ["WorkUnitContext { workUnitId: string; projectId: string; status: string; timestamp: number } @ workunit.service.ts:L15"],',
    '        "testMock": ["vi.mock(\'@dommaker/studio-shared\', () => ({ logger: { info: vi.fn() } }))"],',
    ...
    '      "gotchas": ["潜在坑位"],',
    '      "modelTier": "fast",',
    '      "modelTierReason": "Executor 始终使用 fast 模型"',
    '    }]',
    '  },',
    '  "task": {',
    '    "acGroups": [{',
    ...
    '      }],',
    '      "testFiles": ["__tests__/auth.test.ts", "__tests__/middleware.test.ts"],',
    ...
    '',
    '**测试文件（testFiles）**：Executor 需要运行的已有测试文件路径列表（回归验证）。',
    '- 基于 files 字段推导：同目录 `__tests__/`、import 链关联的测试文件',
    '- Executor 实现代码后运行这些测试验证无回归',
    '- 无关联测试文件时填空数组 `[]`',
    '',
