...
- **消费链路三条已知坑（M2 诊断，2026-09-21，均未修）**：① `verifyConsumptionChain`（knowledge-singletons.ts:137-142，每次进程启动跑）的 `knowledge:probe` 是**直写事件、不走 recordReference**——probe 计数高不代表消费链路通，验收只认 `knowledge:consumption`；② injectContext 以 `wu.type` 作 agentType 过滤 `applicableAgents`（unified-query.ts:71-75），而 rule 条目的 affects 词表是角色词表（agent/reviewer/executor/monitor），与 WU type **零交集**——「## 系统约束」rule 段对任何真实 WU 恒空（潜伏假绿坑）；③ 实盘 63 个 `guideline-rule-*.md` 的 tags 被衰减/lifecycle 循环**无去重反复追加 `deprecated`**（单文件最多千余次，仍在增长）——活跃数据腐蚀 bug，待立案。
