...
- **开图/物化/收尾哨兵**: analysis WU done 且 l3.summary 含 `FOG:`/`DESTINATION:` -> 初始化 PMO map + 逐条建 decision 单(metadata 落 mapOpenedAt 哨兵 + pmoId/fogId 互挂契约); spec WU done 且 l3.summary 含 `TASK:|AC:|BLOCKEDBY:|LEG:` -> 批量建 task 单(specTasksSpawnedAt 哨兵; #463 起无 TASK 行不落哨兵 = 人审有意不物化, 补确认 l3 补写再发 status_changed 可再触发物化); done 触发角色记忆提取(memoryExtractedAt 哨兵)。各哨兵写入走「写入前重读合并」防同事件互覆。
- **#463 确认表单结构化**: review-passed 端点收可选 `confirm` body(decision 结论 / spec 物化 tasks / analysis destination+fog+tasks)——校验与序列化正本在 confirm-payload.ts(resolveReviewConfirm), 后端把表单数据序列化进 l3.summary(存储契约不变, 下游 parseSpecTasks/parseMapOpening/decision-resolution 原样消费), confirm 与裸 summary 并存时 confirm 优先; analysis 的 tasks 经 reviewPassed options.analysisTasks 覆写 metadata.analysisTasks(人审剔除/行内编辑生效)。预填数据源 = agent COMPLETE 落档的 specTasks(spec 输出 TASK 物化行)/decisionSuggestion(decision 输出 `## 结论摘要` 段)——prompt 契约见 agents/loop/prompt-composer CONTRACT_TEMPLATES。
...
- **消息关联 WU 统一路径(#333)**: `createFromMessage` 关联段不再裸 `fileStore.appendMessage`, 改走 `ChannelMessageService.linkWorkUnit`(append 保留 createdAt + eventBus/SSE 双发 `channel.message_updated`, 前端原位刷新); 构造器第二参可注入 messageService(缺省 fileStore 新建, 无 fileStore 用单例, 口径：可注入，缺省 fileStore 新建/无 fileStore 用单例)。
