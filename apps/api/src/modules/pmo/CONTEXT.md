...
| `applyPlanDirection` / `validateDirectionPick` / `PlanDirectionError` | `plan-direction.ts` | #567 方向锁定提交唯一正本：选定方向落探路台账（decisions[] 追加「方向：…（人锁定）」，方向抉择点命中 fog 置 resolved、缺失不补建；幂等不双写）+ 组合选定文本复活 plan 同会话 |
