...
router.get("/:provider(google|github)", (req, res) => {
  ...
  try {
    ...
    logger.error(`[OAuth] Failed to get ${provider} auth URL`, {
      error: message,
    });
...
router.get("/callback/:provider(google|github)", async (req, res) => {
  ...
  try {
    ...
    const { profile, tokens } = await oauthService.exchangeCodeForTokens(
      provider,
      code,
    );

    // Find or create user linked to this OAuth account
    const { user } = await oauthService.getOrCreateOAuthUser(
      provider,
      profile,
      tokens,
    );
    ...
    const { token, refreshToken, session } =
      await oauthService.createOAuthSession(user.id, {
        ip: req.ip,
    ...
    const errorCode = message.includes("exchange")
