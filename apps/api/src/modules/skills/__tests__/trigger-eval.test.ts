...
const ALL_SKILLS: SkillEntry[] = [
  ...
  { name: 'design-analyst', path: 'design-analyst/SKILL.md', description: '需求分析、brainstorming、设计探索、需求澄清、方案对比、产出设计文档。把模糊需求变成结构化设计文档（含方案对比、AC 定义、实施路径）。覆盖：需要澄清的模糊需求、需要架构探索的功能请求、缺乏明确设计方案的任务。不用于审查已有 spec 或设计文档（用 spec-review-skill）、编写 SDD 三层文档（用 task-planner）、实现代码（用 tdd-implement）、审查代码质量（用 code-review）。' },
  ...
  { name: 'task-planner', path: 'task-planner/SKILL.md', description: '任务规划、SDD 编写、实现步骤规划、把设计文档转化为可执行的 SDD（requirement.md + design.md + task.md）。产出包含 AC、文件映射、接口定义、代码依赖、契约测试规划。支持并行探索多个模块。不用于分析需求（用 design-analyst）、审查 SDD 三层文档或三层一致性（用 sdd-review-skill）、实现代码（用 tdd-implement）、审查代码（用 code-review）。' },
  { name: 'tdd-implement', path: 'tdd-implement/SKILL.md', description: '按 SDD 实现代码、写 FAIL 测试、让测试通过、TDD 实现。读取 SDD，按 AC 写 FAIL 测试（RED），再按设计实现代码让测试通过（GREEN）。支持并行实现独立文件、增量类型检查。覆盖：有完整 SDD 的功能实现、需要 TDD 流程的编码任务。不用于分析需求（用 design-analyst）、规划任务（用 task-planner）、审查代码（用 code-review）、审查 SDD 三层文档（用 sdd-review-skill）、或并行执行非实现任务（用 parallel-execution）。' },
