 ...
 *
 * 候选 2：字节级倒读迭代器与尾页读已泛化下沉 @dommaker/studio-shared（jsonl-tail），
 * 本文件只剩 events 专用语义（level 分级 / 时间窗停扫）的薄封装，行切分正确性
 * 论证唯一归属 jsonl-tail。无效 cursor → 从文件尾重新开始；文件不存在 → 空结果。
 */
import { promises as fs } from 'node:fs';
import { iterateJsonlLinesBackward, readJsonlTail } from '@dommaker/studio-shared';
...
export async function readStudioEventsTail(
  ...
  const { rows, nextCursor } = await readJsonlTail({
    file: opts.file,
    limit: opts.limit,
    cursor: opts.cursor,
    match: opts.match,
...
export async function readStudioEventsSince(
  ...
  try {
    ...
    for await (const { text } of iterateJsonlLinesBackward(handle, stat.size, chunkSize)) {
