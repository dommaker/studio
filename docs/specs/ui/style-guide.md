...
> 实现：`apps/web/src/styles/theme.css`（token + 组件类）、`apps/web/src/styles/mission-control.css`（三栏布局 mc-* + 语义工具类 u-*）
...
| `u-mb-3`（批次 D-4） | `margin-bottom: var(--space-3)` 覆盖工具类——theme/mission-control 未分层，Tailwind 工具类压不过 mc-* 类自带 margin 时用（mission-control.css 有注释留痕） |
...
| `empty-state`、`loading-spinner`、`divider` | 见 theme.css |
...
### 4.5 语义工具类 `u-*`（mission-control.css）
...
- **本文档是唯一持续维护的样式规范**；token/组件类变更流程：改 `theme.css`（或 `mission-control.css`）→ 更新本文档 → 在 PR 描述中注明。
