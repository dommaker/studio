...
> 修订：2026-08-16 §1 重写为分层塔（#198–#201 决议，#204 执行）
...
Studio 是一个**多 Agent 群聊控制台**，其价值结构是一座四层塔，下层是上层的前提：

1. **信任基座**（看得见）— 监控、可视化、开销度量：任何任务烧了多少钱、走到哪一步、卡在哪，随时可查；烧钱跑飞时自动熔断。
2. **无人值守异步执行**（敢放手）— 人在频道提出需求，任务派给 agent CLI 自主执行；卡住/拿不准时在频道喊人，人答完继续。派发是这一层的入口动作，不单列。
3. **知识飞轮**（越用越聪明）— 执行数据沉淀为知识，蒸馏后再注入后续执行。
4. **进化**（自己会进化）— 飞轮作用于约束层：系统自己从执行记录提炼新规矩并生效（PatternMiner/traces 同管线）。

③④的分界是「规矩谁定」：知识由人/agent 参考着用是③；系统自提炼的规矩生效是④。

**现实价值 vs 存在理由**：存在理由只锚塔顶两层——飞轮与进化是 studio 区别于「直接在 CLI 里输入需求」的根本；①② 是必经路径，自身有现实价值（飞轮就算死掉，studio 仍是日用的编排+观测台：持久角色、跨任务状态与开销可见），但不构成存在理由。

**信任基座由两条腿构成**：studio 管**全局观测**（事件流、token 账本、熔断），harness 管**现场把关**（单步动作合规，当场拦截）。二者不可互换——整体跑偏只有全局看得见，单步违规只有现场拦得住。
