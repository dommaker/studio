...
1. **发布单元**：单包 `@dommaker/studio`（推荐）vs 多包 publish——影响 Q1/Q9 全部下游。
2. **迁移失败语义**：备份 + 拒启（推荐）vs 备份 + 降级继续跑——拒启更安全，继续跑更不打断用户。
3. **端口冲突默认行为**：动态顺延（推荐）vs 维持拒启 + 提示——动态顺延对新手友好，但"我以为起在 3001"的困惑成本存在。
4. **契约指针挂 AGENTS.md PRESERVE 段**（推荐，治理变更）vs 只留独立文档——前者让契约在 agent 入口文档有强制可见性，但要过人闸。
5. **bin 命令名**：`studio start` 新动词（推荐）vs 沿用 `studio up` 改语义——沿用省一个词表项，但 `up` 现有语义里 monorepo 假设太多，改名比改语义干净。
