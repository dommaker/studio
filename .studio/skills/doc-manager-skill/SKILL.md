...
description: "文档状态管理：保存进度到 memory、维护文档格式、更新 roadmap Phase、同步 spec/SDD status。"
agentTypes: [docs]
...
| code-review 审查通过后自动调用 | update-spec-status | SDD → spec（从 requirement.md 引用定位） |
...
**触发**：
- code-review 审查通过后自动调用（从 SDD requirement.md 的 spec 引用定位）
- 用户说 "更新 spec status" / "同步 spec 状态" / "spec 状态同步"

**输入**：SDD 路径（如 `docs/sdd/spec-2b/`）或 spec 路径（如 `docs/specs/SP-xxx.md`）。

**定位 spec**：
```
1. 如果输入是 SDD 路径 → 读 requirement.md frontmatter 的 `spec` 字段或 body 中的 spec 引用
2. 如果输入是 spec 路径 → 直接使用
3. 找不到 spec 引用 → 跳过（不是所有实现都有对应 spec）
```
...
- [ ] spec 路径是否正确（从 SDD 引用定位或直接指定）
- [ ] 状态转换是否合理（done 不回退，draft→done 需全部 AC 完成）
- [ ] 如果 SDD 存在，SDD status 是否也为 `done`（交叉验证，不一致需排查）
...
| update-spec-status | SDD status=done 但 spec 还是 draft（"改了代码忘了改文档"） | 实现完成后 spec status 必须同步——code-review 通过 = spec→done |
...
2. **确认需求**：
   ...
   - update-spec-status：定位 spec（从 SDD 引用或直接路径），确认当前实现进度
3. **按规范执行**：使用对应的格式和规则
