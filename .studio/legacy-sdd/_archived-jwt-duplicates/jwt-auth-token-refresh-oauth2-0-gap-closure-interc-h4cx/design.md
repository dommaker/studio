...
goalId: "cmq6gvofy01r2qnnhidp6h4cx"
...
#### 实现指南
1. 两处改动都在 oauth.service.ts 的 createOAuthSession 函数内。2. L320: new Date(Date.now() + 24 * 60 * 60 * 1000) → new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)。3. L336: { expiresIn: '24h' } → { expiresIn: '7d' }。4. 不需要改 service.ts（已经是 7 天）。5. 不需要改 Prisma schema（expiresAt 是 DateTime，精度足够）。
