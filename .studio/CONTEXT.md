...
角色身份 = `AgentProfile.id`（agent-loop `this.role.id`）。测试环境经 `isTestEnv` 改写 `os.tmpdir()/studio-test-role-memory/<per-进程子目录>`（同 studio-log-path 约定，防测试写生产 `~/.studio/memory`；per-进程子目录防 vitest 并行测试文件整删互踩，#135）。
