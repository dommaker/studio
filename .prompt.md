...
## 历史相关知识（按需求匹配度排序）
- [REF:ARC-011] 🔍 jwt-认证系统-+-token-刷新-+-oauth2.0-第三方登录: apps/web/src/api/index.ts, apps/web/src/stores/authStore.ts
- **lurk-wall**: apps/api/src/app.ts
- **rate-limiting**: apps/api/src/modules/auth/routes.ts, apps/api/src/middleware/rate-limit.ts
- **csr
- [REF:AS-019-knowledge-engine-search] 🔍 AS-019-knowledge-engine-search:  模糊匹配 | 人 (前端) | 只查 Wiki 文档，不查 KnowledgeStore |
| **CST** | **无通道** | **开发者** | **飞轮的知识开发者查不到** |

### 受众需求矩阵

| 受众 | 查询方式 | 返回量 | 延迟要求 |
|------|---------|:---:|:---:|
| Agent (Analyst/Executor) | 关键
