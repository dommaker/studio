...
- [REF:ARC-013] 🔍 b51-管线端到端验证-—-e2e-verification-常量与测试: ## B51v3 管线端到端验证常量与测试
在 health 模块新建 PIPELINE_E2E_B51_V3 常量文件并编写对应 vitest 测试

### Modules Analyzed
- **e2e-constant-and-test**: apps/api/src/modules/health/pipeline-e2e-b51.ts
- [REF:ARC-015] 🔍 jwt-认证系统测试覆盖补全-—-auth-+-oauth2.0-+-token: ## JWT 认证系统测试覆盖补全 + 边界加固
核心 JWT 认证系统（token 签发/验证、refresh token 轮转、OAuth2.0 Google/GitHub 登录、中间件链、速率限制、CSRF 防护）已完整实现——8 个 AC 组覆盖全部功能模块，含单元测试 28+13+7=48 用例 + E2E 14 用例。本次需求聚焦剩余缺口：路由层测试、OAuth exchange
...
**已验证文件** (不需要重新探索): apps/api/src/middleware/auth.ts
