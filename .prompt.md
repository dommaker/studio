...
基于JWT的用户认证系统，支持 access/refresh token 双令牌轮转、Google/GitHub OAuth2.0 第三方登录、前端 axios 拦截器自动刷新、Lurk Wall 全局认证墙、速率限制、CSRF 防护、会话管理。已完整实现——8个 A
- [REF:GUI-017] 🔍 [Analyst] JWT 用户认证系统 + Token 刷新 + OAuth2.0 第三方登录: 原子 token 轮换 — prisma.$transaction 包裹：吊销旧 refreshToken → 签发新 refreshToken → 生成新 J: Refresh Token 轮换 (Rotation): exchangeRefreshToken 吊销旧 token → 创建新 access token + 新 refresh token。前端 axios interceptor 401 → refresh → retry + 并发队列。前端用独立 axios 实
- [REF:GUI-018] 🔍 [Analyst] JWT 认证系统 + Token 刷新 + OAuth2.0 第三方登录: 分层过期策略：Refresh token 30 天 > JWT 7 天 > Guest session 24h: 分层过期策略：Refresh token 30 天 > JWT 7 天 > Guest session 24h
...
**已验证文件** (不需要重新探索): apps/api/src/modules/auth/service.ts, apps/web/src/api/index.ts
