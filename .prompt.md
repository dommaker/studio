...
## 项目上下文
## 上下文
- 偏好模型: standard；置信度: 0.992
- 平台: linux；Node: v22.22.0；环境: development；已知限制: [object Object], [object Object]

## 近期信号
- [GUI-034] vendor_error (degraded)
Incident: I-20260610-fa5z
- [PIT-004] ⚠️ OAuth state cookie must be httpOnly + sameSite:'lax' + maxAge 600s — CSRF protection
- [PIT-009] express-rate-limit 默认使用 req.ip 作为 key。如果部署在反向代理（nginx/cloudflare）后面且未设置 trust proxy，所有请求会被识别为同一 IP，限
- [PRO-003] Scope: create-test-sse3.txt
Action: created
Entry: ARC-015
Content size: 201B
Duration: 409ms
- [PRO-004] Stale entries: 0
Unmonitored: 5
Healed: 0
Duration: 266ms
Unmonitored: 修复-session-manager.ts-+-agent

[知识库: 30 条参考，遇到问题时用 search()]

知识库: 38 条 | 代码模式 5 | 坑点 2 | 修复方案 1 | 趋势 2
## 历史相关知识（按需求匹配度排序）
- [REF:GUI-010] 🔍 [Session Fix] fix: B8 cacheHitRate 公式修正 — cacheHit/(cacheHit+input) 替代 cacheHit/input: Commit: 403d82df
Message: fix: B8 cacheHitRate 公式修正 — cacheHit/(cacheHit+input) 替代 cacheHit/input
Files: apps/api/src/modules/pmo/okr.service.ts
Pattern: B8 cacheHitRate 公式修正 — cacheHit/(cacheHit
- [REF:GUI-012] 🔍 [Session Fix] fix: replace uuid with crypto.randomUUID — no external dep: Commit: 40a43295
...
Patte
- [REF:ARC-011] 🔍 jwt-用户认证系统:-token-刷新-+-oauth2.0-第三方登录: ## JWT 用户认证系统: Token 刷新 + OAuth2.0 第三方登录
完整的 JWT 认证系统：bcrypt 密码加密、JWT 签发/验证、refresh token 轮转、Google/GitHub OAuth2.0、前端自动刷新拦截器、RBAC 中间件

### Modules Analyzed
- **jwt-core**: apps/api/src/modul
- [REF:ARC-015] 🔍 create-test-sse2.txt: ## Create test-sse3.txt
Create test-sse3.txt file with content 'SSE Test 3'

### Modules Analyzed
- **create-file**: test-sse3.txt

### Key Patterns
- 参考 test-sse.txt 和 test-sse2
- [REF:PIT-004] ⚠️ [Analyst] JWT + Token Refresh + OAuth2.0 — Remaining Implementation (v13): ⚠️ OAuth state cookie must be httpOnly + sameSite:'lax' + maxAge 600s — CSRF pro: ⚠️ OAuth state cookie must be httpOnly + sameSite:'lax' + maxAge 600s — CSRF protection
...
- **GREEN-Only TDD**: GREEN-only TDD 工作流：读 Analyst 契约测试→实现代码→确认通过→重构
