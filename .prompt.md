...
## 历史相关知识（按需求匹配度排序）
- [REF:GUI-009] 🔍 Review: APPROVED (cycle 1, score 80): 83c7zfok
Issues: 8
Score: 80

[warning] exchangeGoogleCode/exchangeGitHubCode untested — only exchangeCodeForTokens('unsupported') has test. Network calls (fetch) not mocked.
[warning] Refresh token r
- [REF:AS-019-knowledge-engine-search] 🔍 AS-019-knowledge-engine-search: other: 1 }
  freshness  = daysAgo < 7 ? 1.0 : max(0.2, 1 - (daysAgo - 7) / 30)
  maturity   = { canonical: 1.5, verified: 1.0, draft: 0.5 }

  return keywordMatch * typeWeight * freshness * maturity
`
...
**已验证文件** (不需要重新探索): apps/api/src/modules/auth/oauth.service.ts
