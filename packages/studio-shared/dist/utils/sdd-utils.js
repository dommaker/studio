 ...
 */
export function parseSddFrontmatter(content) {
 ...
 */
export function stringifySddFrontmatter(fm) {
    const lines = [];
...
function getSddBaseDir() {
    if (process.env.SDD_DIR)
        return process.env.SDD_DIR;
 ...
 * 写入 SDD 文档。
 */
export async function writeSddDoc(slug, layer, frontmatter, body) {
    await store.writeDoc(getSddBaseDir(), `${slug}/${layer}`, frontmatter, body);
}
/**
 * 列出所有 SDD 文档目录（扫描子目录，非 flat .md 文件）。
 */
export async function listSddDocs() {
...
export async function findSddDocById(id) {
...
export async function findSddDocByWorkUnitId(workUnitId) {
    const slugs = await listSddDocs();
    for (const slug of slugs) {
        const doc = await readSddDoc(slug, 'requirement');
        if (doc?.meta.workUnitId === workUnitId)
            return slug;
    }
    return null;
}
export async function readSddDocByWorkUnitId(workUnitId, layer) {
...
export function parseTaskDocContractTests(body) {
    const results = [];
...
export function parseTaskDocTestFiles(body) {
    const lines = body.split('\n');
    const results = [];
...
export async function appendChangelog(slug, entry) {
    await store.appendChangelog(getSddBaseDir(), slug, entry);
}
// ── SddRepository 查询/更新 ──
export async function findSddDocs(filter) {
    const slugs = await listSddDocs();
    const results = [];
    for (const slug of slugs) {
        const doc = await readSddDoc(slug, 'requirement');
        ...
        if (filter?.status && doc.meta.status !== filter.status)
            continue;
        if (filter?.workUnitId && doc.meta.workUnitId !== filter.workUnitId)
            continue;
        results.push(doc.meta);
    }
    return results;
}
export async function updateSddFrontmatter(slug, patch) {
    const baseDir = process.env.SDD_DIR || 'docs/sdd';
    const doc = await store.readDoc(baseDir, `${slug}/requirement`);
    if (!doc)
        throw new Error(`SDD doc not found: ${baseDir}/${slug}/requirement.md`);
    ...
    if (Object.keys(doc.meta).length === 0 && !doc.body.startsWith('---')) {
        ...
        if (!rawParsed)
            throw new Error(`Invalid frontmatter in: ${baseDir}/${slug}/requirement.md`);
    }
    const merged = { ...doc.meta, ...patch };
    await store.writeDoc(baseDir, `${slug}/requirement`, merged, doc.body);
}
//# sourceMappingURL=sdd-utils.js.map