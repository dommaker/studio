 ...
 */
export function getDispatchStrategy(recentFailures, recentTotal) {
    if (recentTotal < 5)
        return 'normal';
 ...
 */
export function getAvailableSlots(maxCap) {
 ...
 */
export function updateDispatchOutcome(state, success) {
    let { failures, total } = state;
    failures += success ? 0 : 1;
    total++;
    if (total > 20) {
        total = 20;
        failures = Math.min(failures, 20);
    }
