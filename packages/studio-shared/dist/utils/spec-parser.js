 ...
 */
export function parseSpecMarkdown(content, filePathOrId) {
    ...
    if (acceptanceCriteria.length > 0)
        spec.acceptance_criteria = acceptanceCriteria;
 ...
 * 解析元数据
 */
function parseMetadata(lines, filePath) {
    // 从标题提取 ID: # DD-006: ...
    ...
    let status;
    if (statusLine) {
        if (statusLine.includes('已实现') || statusLine.includes('已完成'))
            status = 'completed';
        else if (statusLine.includes('进行中'))
            status = 'in_progress';
        else if (statusLine.includes('废弃') || statusLine.includes('已弃用'))
            status = 'deprecated';
        else
            status = 'draft';
    }
 ...
 * 解析 API endpoints
 */
function parseApiEndpoints(lines) {
    const endpoints = [];
    // 找到 API 设计段落中的表格
