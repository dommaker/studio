...
async function callAnthropicProvider(provider, messages, options) {
    ...
    return {
        ...
        choices: data.content?.map((block) => ({
        ...
        usage: data.usage ? {
            ...
            cache_read_tokens: data.usage.cache_read_input_tokens || data.usage.cache_hit_tokens || 0,
        } : undefined,
    };
}
// ─── Prompt 缓存 ───
 ...
 * 统一调用入口（带 fallback）
 */
export async function chat(request, providers, usageLogRef, promptCache) {
    ...
    for (const provider of resolved) {
        ...
        try {
            ...
            const response = { content, model: raw.model || provider.model, provider: provider.name, usage, latencyMs };
            if (useCache) {
 ...
 * 简单调用（单条消息）
 */
export async function prompt(text, systemPrompt, chatFn, options) {
    const messages = [];
    if (systemPrompt)
 ...
 * 结构化输出（JSON）
 */
export async function promptJson(text, systemPrompt, chatFn, options) {
