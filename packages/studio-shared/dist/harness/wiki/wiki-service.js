...
function wikiRoot(companyId) {
    return path.join(BASE_DIR, companyId, 'wiki');
}
/**
 * 写入 Wiki 页面
 */
export function writeWikiPage(companyId, page) {
 ...
 * 读取 Wiki 页面
 */
export function readWikiPage(companyId, relativePath) {
 ...
 */
export function createProjectPage(companyId, pmoNumber, data) {
 ...
 * 追加到 INDEX.md
 */
function appendToIndex(companyId, pmoNumber, title) {
 ...
 * 列出公司所有 Wiki 页面
 */
export function listWikiPages(companyId) {
    const root = wikiRoot(companyId);
 ...
 */
export function updateProjectPageExecutionResult(companyId, pmoNumber, result) {
