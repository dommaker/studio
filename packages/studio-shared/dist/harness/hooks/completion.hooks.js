...
/** 任务完成前：测试门控 */
export async function checkBeforeTaskComplete(testResults, config) {
    const gate = new PassesGate(config);
    // Pass the first test result directly — gate.check() expects {passed, evidence}, not {testResults: [...]}
    const result = await gate.check(testResults[0] || { passed: false, command: 'unknown' });
    return {
...
//# sourceMappingURL=completion.hooks.js.map