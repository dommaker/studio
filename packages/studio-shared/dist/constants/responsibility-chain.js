 ...
 * 按重要性排序：越靠前越关键
 */
export const RESPONSIBILITY_CHAIN = {
 ...
 * 这些角色会追加到基础责任链
 */
export const CHANGE_TYPE_EXPERTS = {
    // 高风险变更：需要架构师
    database: ['architect'],
 ...
 * @returns 需要参与审批的角色列表
 */
export function decideParticipants(stage, constraintLevel, changeTypes) {
    // 1. L1 特殊处理：自动批准
 ...
 * @returns 是否有责任
 */
export function canRoleExecuteStage(role, stage) {
 ...
 */
export function getRoleDepthInStage(role, stage) {
    const chain = RESPONSIBILITY_CHAIN[stage] || [];
 ...
 * @returns 是否可用
 */
export function isToolAllowedForStage(toolPath, stage) {
    const allowedPatterns = STAGE_TOOLS[stage] || [];
    for (const pattern of allowedPatterns) {
        if (pattern.endsWith('*')) {
            // 通配符匹配
            const prefix = pattern.slice(0, -1);
            if (toolPath.startsWith(prefix))
                return true;
        }
        else {
            // 精确匹配
 ...
 * - 可使用的 Tools
 */
export function deriveRoleConfig(role) {
    // 1. 找出有责任的阶段
    const stages = [];
 ...
 * 从配置自动聚合，不硬编码
 */
export function buildUICategories() {
    const categories = [];
...
//# sourceMappingURL=responsibility-chain.js.map