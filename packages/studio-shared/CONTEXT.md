...
| hooks 管线集成层 | `src/harness/hooks/`（config/register/*.hooks.ts）+ `src/harness/runtime/bootstrap.ts` | #159（harness 1.8.0）：`config.ts` 声明表是 enabled/errorStrategy **唯一声明点**（HARNESS_HOOK_DISABLE env 覆盖 + blocking→errorStrategy 映射源）；`registerAllHooks` = `registry.registerAll(defs, configs)` 配对注册，有效值由 harness 填充（EffectiveHook）。**判定一处声明、两处读取**：① harness 管线按注册表有效值过滤/分派，粒度是**时机**（`run('before'｜'after')` 跑该时机全部 hook）；② `config.ts` 的 `runHook` 供管线外**按名字**直调。studio 的真实生产入口是 ②——`packages/studio-agent/src/services/runner-execution.ts:101` 直调 `beforeAgentExecute`，不经管线（管线本身在 studio 生产侧零调用方）。把 hook 函数写成纯实现体（1.8.0 随动初版）等于 `enabled`/`HARNESS_HOOK_DISABLE` 在这条路径上配了不生效，已由 `src/harness/hooks/__tests__/direct-call-gate.test.ts` 钉住（含「摘掉判定层必红」的反证）。根治 = harness 补「按名执行单 hook 且带 enabled/strategy」的公共面后删除 ②，见 harness#167。闭环由 `scripts/tools/hooks-closure-check.ts` 挂 build 断言 |
