...
| hooks 管线集成层 | `src/harness/hooks/`（config/register/*.hooks.ts）+ `src/harness/runtime/bootstrap.ts` | #159（harness 1.8.0）：`config.ts` 声明表是 enabled/errorStrategy **唯一声明点**（HARNESS_HOOK_DISABLE env 覆盖 + blocking→errorStrategy 映射源）；`registerAllHooks` = `registry.registerAll(defs, configs)` 配对注册，有效值由 harness 填充（EffectiveHook）、**判定只住 harness 管线**——原 `runHook` 自建判定层已拆除，hook 函数是纯实现体；直接调 hook 函数 = 绕过 enabled/block/warn 判定（生产路径经 bootstrap 注册后由管线执行），闭环由 `scripts/tools/hooks-closure-check.ts` 挂 build 断言 |
