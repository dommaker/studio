 ...
 *
 * #361 自 apps/api utils/studio-events.ts 下沉：此前实现住在 apps/api，
 * studio-agent（output-capture 5 个同构 emit）与 skills 直写够不着只能自抄
 * appendJsonl，且绕过 STUDIO_EVENTS_FILE 测试隔离 → vitest 下 runner 事件
 * 落生产 logs。收一后所有调用方经本模块写入。
 *
 * 背景（裂口）：历史上存在两条事件流 ——
 ...
 * log-path 隔离到 os.tmpdir()/studio-test-logs/）+ 一个入口（writeStudioEvent /
 ...
 *
 * #173（#60 决策 Q3b）：轮转保留在 apps/api ./studio-events-rotation.ts（信号热
