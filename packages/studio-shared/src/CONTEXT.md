...
| `resolveVpsWorkspace`, `resolveWorkspacesDir` | vps-workspace（仅 /node 入口） | 'VPS' 工作区命名约定与 ~/.studio/workspaces 扫描的唯一属主（2026-08 起，禁止第三处手扫）。现消费方 = apps/api `local-workspace` 与 `GET /workspaces/runtimes`；2026-09-10 起 worktree-resolver 不再走它——记录 root 退出执行面，无归属任务落隔离 scratch |
