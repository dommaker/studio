...
describe('propagateHarnessConfig → 停写 AGENTS.md / CLAUDE.md / skill 拷贝（决策 6）', () => {
  ...
  test('仍写 .claude/settings.json（bypassPermissions + studio MCP）', async () => {
    await propagateHarnessConfig(worktree, 'task-1', 'exec-1');
  ...
  test('settings.json 已存在时不重写（保留既有 MCP 配置）', async () => {
    ...
    expect(fs.readFileSync(path.join(worktree, '.claude', 'settings.json'), 'utf-8')).toBe('{"custom":true}\n');
  });
});
