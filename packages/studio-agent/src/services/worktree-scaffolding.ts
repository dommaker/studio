/**
 * Worktree Scaffolding — worktree 内的脚手架写入：REQUIREMENTS.md / 契约测试 / 依赖缓存安装
 *
 * 从 worktree-resolver.ts 拆出（纯移动，逻辑零改写）；
 * worktree-resolver.ts 门面 re-export 本模块全部导出，消费方导入路径不变。
 */
