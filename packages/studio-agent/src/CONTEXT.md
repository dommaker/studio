...
- **执行目录解析（resolveWorkspace，2026-09-10 重排）**：① `task.parameters.workspaceRoot`（上游归属信号：@文件引用 / PMO 项目 gitRepo / 频道默认工程）→ ② `hasWorktree=true` 建隔离 worktree → ③ 都没有则落 `scratch`（缺省 `studioPath('scratch')/<executionId>`，测试可经 `scratchDir` 覆盖），**不再指向任何真实仓**。原 ②（2026-08 seam 修复时是「调 `resolveVpsWorkspace()` 取本机 VPS 记录的 root」）已删除：那条 root 来自最早启动的服务器进程的 `REPO_DIR`/cwd，生产 unit 写的是开发工作副本，等于让无归属任务悄悄在别人的代码目录里跑（证据形态：执行日志的 cwd 落在服务器自身的源码工作副本、甚至其上层目录；细节见本地 `docs/plans/2026-09-deprecated-capability-retirement.md`，该目录不入库）。配套：`ensureDeps` 增前置条件——目录内既无 lockfile 又无 package.json 即判定非项目检出直接返回，否则会拿 repoDir 的 lockfile 把 node_modules 硬链进空 scratch。`'VPS'` 命名约定的唯一属主仍在 studio-shared（现消费方 = apps/api local-workspace 与 `GET /workspaces/runtimes`）。
