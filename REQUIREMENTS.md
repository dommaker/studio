...
## 验收标准
1. 为 authStore.ts (215行 Zustand store) 编写单元测试：覆盖 login/register/logout/refreshToken/getCurrentUser action + 状态变更
2. 为 OAuthCallback.tsx (50行) 编写组件测试：覆盖 URL fragment 解析 → token 存储 → redirect 流程、错误 fragment 展示
3. 为 axios interceptor 编写单元测试：覆盖 Bearer token 注入、401 → refresh → retry 流程、refresh 失败清除 auth state
